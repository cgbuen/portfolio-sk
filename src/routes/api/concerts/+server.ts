...
export async function GET() {
  ...
  let response = photographyResponseJson
    ...
    .map((photo: Photo) => {
      ...
      photo.alt2 =
        photo.city === 'Coachella'
          ? `${photo.city} ${photo.date.substring(0, 4)}`
          : `${photo.city}, ${MONTHS[parseInt(photo.date.substring(5, 7)) - 1]} ${photo.date.substring(0, 4)}`;
